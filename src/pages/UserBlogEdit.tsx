...
export default function UserBlogEdit() {
  ...
  const fetchPost = async () => {
    try {
      ...
      setOriginalPost(data as BlogPost);
      setFormData({
        title: data.title,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const successMessage = blogSettings?.require_approval && formData.status === 'published'
        ? "Blog post updated and submitted for approval"
    ...
    return (
      <FeatureAccessGuard feature="blog">
        <div className="text-center py-8">
          ...
          <Button onClick={() => navigate('/user-blog-management')} className="mt-4">
