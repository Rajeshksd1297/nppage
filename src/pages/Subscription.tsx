...
export default function Subscription() {
  ...
  const fetchData = async () => {
    try {
      ...
      const {
        data: booksData
      } = await supabase.from('books').select('status').eq('user_id', user.id);
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 max-w-7xl">
      ...
      <div className="text-center mb-12">
        ...
        <h1 className="font-bold mb-6 bg-gradient-to-r from-primary via-purple-600 to-primary bg-clip-text text-transparent text-3xl">Choose Your Plan</h1>
      ...
      <Tabs defaultValue="current" className="w-full">
        ...
        <TabsContent value="current" className="space-y-6">
          {subscription ? <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-purple-600/5">
              <CardHeader>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      {subscription.subscription_plans.max_books !== -1 && <Progress value={getPlanUsagePercentage()} className="h-2" />}
                      {subscription.subscription_plans.max_books === -1 && <div className="text-center p-2 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    {subscription.subscription_plans.max_books !== -1 && userStats.totalBooks >= subscription.subscription_plans.max_books && <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
        ...
        <TabsContent value="packages" className="space-y-6">
          ...
          <div className={`grid gap-6 mx-auto ${
            ...
            return <Card key={plan.id} className={`relative transition-all duration-300 hover:shadow-xl ${isPremium ? 'border-primary shadow-lg scale-105' : 'hover:scale-102'} ${isCurrentPlan || isOnTrialForPlan ? 'ring-2 ring-primary' : ''}`}>
                  ...
                  <CardHeader className="text-center pb-6 pt-8">
                    <div className="mb-4">
                      {isPremium ? <div className="w-16 h-16 bg-gradient-to-br from-primary to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
                          <Zap className="w-8 h-8 text-white" />
                        </div> : <div className="w-16 h-16 bg-muted rounded-2xl flex items-center justify-center mx-auto mb-4">
                    ...
                    <div className="mb-6">
                      ...
                      {billingCycle === 'yearly' && plan.price_monthly > 0 && <p className="text-sm text-muted-foreground mt-2">
                          ${(plan.price_monthly * 12).toFixed(0)} billed annually
                        </p>}
                      {isOnTrialForPlan && <Badge variant="outline" className="mt-3 bg-amber-50 text-amber-700 border-amber-200">
                  ...
                  <CardContent className="pt-0">
                    ...
                    <Button className={`w-full h-12 ${isPremium ? 'bg-gradient-to-r from-primary to-purple-600 hover:from-primary/90 hover:to-purple-600/90' : ''}`} variant={isPremium ? "default" : "outline"} disabled={isCurrentPlan || isOnTrialForPlan} onClick={() => handleUpgrade(plan.id)}>
                      {isCurrentPlan ? <>
                ...
                <div className="min-w-[600px]"> {/* Minimum width for proper table display */}
                  ...
                  <thead>
                    <tr className="border-b">
                      ...
                      {plans.map(plan => <th key={plan.id} className="text-center p-4">
                  ...
                  <tbody>
                    ...
                    {premiumFeatures.map((feature, idx) => <tr key={`premium-${idx}`} className="border-b hover:bg-muted/50">
                        <td className="p-4 font-medium">{feature.name}</td>
                        {plans.map(plan => {
                      const planFeatures = getPlanFeatures(plan.id);
                      const hasFeature = planFeatures.some(f => f.id === feature.id);
